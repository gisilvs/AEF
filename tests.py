...
def verify_inverse_nae(nae: NormalizingAutoEncoder, num_channels=3, eps=1e-6):
...
def verify_forward_nae(nae: NormalizingAutoEncoder, num_channels=3, eps=1e-6):
    input = torch.rand(4, num_channels, 28, 28)
...
def verify_inverse_flow(flow, eps=1e-6):
...
decoder = Decoder(64, 12, [3,28,28])
mask = torch.zeros(28,28)
mask[13:15,13:15] = 1
nae = NormalizingAutoEncoder(core_flow, encoder, decoder, mask)
print(f"NAE (inverse -> forward) returns input: {verify_inverse_nae(nae)}")
print(f"NAE (forward -> inverse) returns input: {verify_forward_nae(nae)}")
print(f"RealNVP returns input: {verify_inverse_flow(core_flow)}")